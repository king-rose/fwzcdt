...
import com.agree.system.mapper.SysUserMapper;
...
public class ProcessServiceImpl implements IProcessService {
    ...
    public <T> void submitApply(T entity, String key, Map<String, Object> variables) throws Exception {
        ...
        if (clazzSimpleName.equals("AesbGDemandInfo")){
            Method getId = clazz.getDeclaredMethod("getDemandId");
            String id =  getId.invoke(entity).toString();

            Method setUpdateBy = clazz.getSuperclass().getDeclaredMethod("setUpdateBy", String.class);
            Method setUpdateTime = clazz.getSuperclass().getDeclaredMethod("setUpdateTime", Date.class);
        ...
        if (clazzSimpleName.equals("BizSimpleTest")){
        ...
        if (clazzSimpleName.equals("BizLeave")){
            Method getId = clazz.getDeclaredMethod("getId");
            Long id = (Long) getId.invoke(entity);
        ...
        if (clazzSimpleName.equals("OprScenarioVersion")){
            Method getSvId = clazz.getDeclaredMethod("getSvId");
            String svId = (String) getSvId.invoke(entity);

            Method setSvUpdateUser = clazz.getDeclaredMethod("setSvUpdateUser", String.class);
            Method setSvUpdateTime = clazz.getDeclaredMethod("setSvUpdateTime", Date.class);
            ...
            setApplyTime.invoke(entity, now);
            setProcessKey.invoke(entity, key);
            setSvUpdateUser.invoke(entity, username);
            setSvUpdateTime.invoke(entity, now);

            // 用来设置启动流程的人员ID，引擎会自动把用户ID保存到activiti:initiator中
            identityService.setAuthenticatedUserId(username);
            // 启动流程时设置业务 key
            ProcessInstance instance = runtimeService.startProcessInstanceByKey(key, svId + "", variables);
            // 更新业务表流程实例id字段
            setInstanceId.invoke(entity, instance.getId());
            // 记录流程实例业务关系
            InstanceBusiness ib = new InstanceBusiness();
            ib.setInstanceId(instance.getId());
            ib.setBusinessKey(svId + "");
            ib.setModule(humpToLine(entity.getClass().getSimpleName()).substring(1));
            taskMapper.insertInstanceBusiness(ib);
        }
    }
    ...
    public TableDataInfo findTodoTasks(TaskVo taskVo) {
        ...
        if (!CollectionUtils.isEmpty(tasks)) {
            tasks.forEach(task -> {
                ...
                System.out.println("ibMap = " + ibMap);
                //创建空map集合
                Map<String, Object> formData = new HashMap<>();
                //补充：不同流程需要判断key
                if (ibMap.get("MODULE").toString().equals("opr_scenario_version")) {
                    formData = taskMapper.selectBusinessByBusinessKeyAndModule(ibMap.get("BUSINESS_KEY").toString(), ibMap.get("MODULE").toString());
                }else if (ibMap.get("MODULE").toString().equals("aesb_g_demand_info")){
                    formData = taskMapper.selectBusinessByBusinessKeyAndModuleToDemand(ibMap.get("BUSINESS_KEY").toString(), ibMap.get("MODULE").toString());
                }else if (!ibMap.get("MODULE").toString().equals("opr_scenario_version")&&!ibMap.get("MODULE").toString().equals("aesb_g_demand_info")){
                    formData = taskMapper.selectBusinessByBusinessKeyAndModuleToOther(ibMap.get("BUSINESS_KEY").toString(), ibMap.get("MODULE").toString());
                }
                if (!CollectionUtils.isEmpty(ibMap)) {
                    if (!CollectionUtils.isEmpty(formData)) {
                        newTaskVo.setFormData(getLine2HumpMap(formData));
                    }else {
                        newTaskVo.setFormData(new HashMap());
                    }
                }
    ...
    public void complete(String taskId, String instanceId, String variablesStr) {
        ...
        try {
            ...
            // 所以在 complete 之前需要先 r run esolved
            ...
            //获取业务key以及对应表
            Map<String, Object> ib = taskMapper.getInstanceBusinessByInstanceId(instanceId);
            String businessKey = ib.get("BUSINESSKEY").toString();
            String module = ib.get("MODULE").toString();
            //查询历史流程实例表判断审核状态
            String EndActId = taskMapper.getEndActIdByInstanceIdAndBusinessKey(instanceId,businessKey);
            if (!EndActId.isEmpty() || EndActId.equals("endEvent")){
                //修改场景版本审核状态
                if (variables.get("pass").toString().equals("true")){
                    taskMapper.updateAuditState(businessKey,module,"2");
                }else {
                    taskMapper.updateAuditState(businessKey,module,"3");
                }
    ...
    public List<HistoricActivity> selectHistoryList(HistoricActivity historicActivity) {
        ...
        HistoricActivityInstanceQuery query = historyService.createHistoricActivityInstanceQuery(); //查询ACT_HI_ACTINST表
        ...
        HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()//查询ACT_HI_PROCINST表
    ...
    public HistoricProcessInstance selectProcessByInstanceId(String pProcessInstanceId) {
        return taskMapper.selectProcessByInstanceId(pProcessInstanceId);
    }
