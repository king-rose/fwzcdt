<template>
    <div>
        ...
        <el-button
            :style="row.suspendState === '2' ? 'color: #13ce66':'color: #ff2c50'"
            size="mini"
            type="text"
            icon="el-icon-sort"
            @click="handleState"
            v-show="row.taskId !== '-1' && row.taskId !== '-2' && type === 'none'"
        >{{ row.suspendState === '2' ? '激活' : '挂起' }}
        </el-button>
        ...
        ></simple-form>
        <audit-form
            v-if="row.processKey === 'sceneRelease'"
            :row="row"
            :taskId="taskId"
            :taskName="taskName"
            :type="type"
        ...
        <el-dialog title="选择用户" :visible.sync="showUserTable" width="900px" append-to-body>
            <!--用户列表表格-->
            <div>
                <el-form :model="queryUserParams" ref="queryUserForm" :inline="true" v-show="showUserSearch"
                         label-width="68px">
...
<script>
    ...
    import simpleForm from "@/views/simple/simple/simpleForm";
    import auditForm from "@/views/manager/service/auditForm";
	import demandForm from "@/views/demand/demandForm";


    export default {
        name: "ApplyAfter",
        components: {DemoForm, LeaveForm,simpleForm,auditForm,demandForm},
        props: {
            /* 表格行数据 */
            row: {
                type: Object,
            ...
            _handleView: function () {
                switch (this.row.processKey) {
                    ...
                    case 'leave':
                        ...
                        break;
                    case 'process':
                        this.$refs.simpleForm.form = this.row;
                        this.$refs.simpleForm.open = true;
                        break;
                    case 'sceneRelease':
                        this.$refs.auditForm.form = this.row;
                        this.$refs.auditForm.open = true;
					case 'demand':
						this.$refs.demandForm.form = this.row;
						this.$refs.demandForm.open = true;
                    default:
                        break;
                }
            },
