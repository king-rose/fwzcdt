<template>
    <div class="app-container">
        ...
        <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
                <el-button
                    ...
                    :disabled="multiple"
                    @click="handleDelete"
                    v-hasPermi="['simple:simple:remove']"
                >删除
                </el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    size="mini"
                    @click="handleExport"
                    v-hasPermi="['simple:simple:export']"
                >导出
                </el-button>
            </el-col>
        ...
        <el-table v-loading="loading" :data="simpleList" @selection-change="handleSelectionChange">
...
<script>
    import {listSimple, getSimple, delSimple, addSimple, updateSimple, exportSimple} from "@/api/simple/simple";
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                this.loading = true;
                listSimple(this.queryParams).then(response => {
                    this.simpleList = response.rows;
                ...
                this.$refs["form"].validate(valid => {
                    if (valid) {
                        if (this.form.id != null) {
                            updateSimple(this.form).then(response => {
                                this.msgSuccess("修改成功");
                                this.open = false;
                                this.getList();
                            });
                        } else {
                            addSimple(this.form).then(response => {
            ...
            handleDelete(row) {
                ...
                this.$confirm('是否确认删除请假测试编号为"' + ids + '"的数据项?', "警告", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(function () {
                    return delSimple(ids);
                }).then(() => {
                    this.getList();
            ...
            handleExport() {
                ...
                this.$confirm('是否确认导出所有请假测试数据项?', "警告", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(function () {
                    return exportSimple(queryParams);
                }).then(response => {
